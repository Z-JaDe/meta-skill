...
**审计产出（必须）**:
- `pressure_scenarios`: 每个场景的压力类型、A/B/C 选项、预期正确选项
- `rationalization_log`: 逐字说辞原文 + 分类 + 触发场景
- `countermeasure_map`: 说辞 -> 对应封堵规则（No exceptions + 禁止变通）
...
| 错误/情况 | 修复/处理 | 停止重做 |
|-----------|-----------|----------|
| 压力场景单一 / 压力<3 种 | 叠加≥3 种压力 | ✓ |
| 说辞概括记录 / 概括/省略 | 逐字记录原文 | ✓ |
| 反制措施模糊 / 留漏洞 | 明确禁止每种变通 | ✓ |
| 未使用说服原则 | 权威 + 承诺 + 社会证明 | - |
| 未设计 Red Flags | 将说辞转化为 Red Flag | - |
| 缺少审计产出（scenarios/log/map） | 补齐三类产出后再继续 | ✓ |
...
# 可选：cat evals/evals.json | jq '.evals[].pressure_scenarios'  # 若项目有 evals 目录
```

**部署检查**（按阶段 1-5 逐项）:
- [ ] 阶段 1: 压力≥3 种、含折中陷阱
- [ ] 阶段 2: 说辞逐字记录
- [ ] 阶段 3-4: 说服原则（权威+承诺+社会证明）、漏洞封堵（No exceptions + 逐一禁止）、Red Flags
- [ ] 阶段 5: 重测无新说辞
